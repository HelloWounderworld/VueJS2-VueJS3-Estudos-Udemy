...
// Existe uma forma ainda melhor para acessar os recursos do vuex, fora o namespaced. No caso, a forma para isso seria o createNameSpaceHelpers que podemos chamar do vuex
// Em aplicacoes de grande porte, a forma como foi organizada as funções que foram definidas dentro das funcionalidades do index.js store e do diretorio module, acaba não sendo o suficiente quando se trata de criar algo em nível empresarial
// No caso, para isso iremo utilizar uma outra forma chamada recurso pela pasta resources que foi criado
// Agora, vamos mostrar como podemos registrar os módulos dinamicamente pelo tarefasModule importado e pelo this.$store.unregistrerModule
// import tarefasModule from './../_store/index'
// Existe uma forma de verificar se estamos causando algum mutação fora do escopo que estamos utilizando. Algo que é muito ruim na prática
// Essa forma de verificação é pelo strict usado no index store, com o booleano.
// Entretanto, habilitar a tal strict por muito tempo lançado em produção tornaria o processamento muito custoso
// Para isso, vamos precisar que essa funcionalidade esteja funcionando somente quando estivermos em ambiente develop, e não em produção
// No caso, usaremos umacondicional dentro do stric como é visto process.env.NODE_ENV
...
// No caso, os mapActions, mapGetters e mapStates dentro do objeto definido na const estão chamando as funcionalidades definidas e sendo chamadas pela instância tarefas no index
...
export default {
  ...
  created () {
    // this.$store.unregisterModule(['tarefas', 'subtarefas']) // tarefas/subtarefas
    // console.log('Store: ', this.$store)
    // this.$store.unregisterModule('tarefas', tarefasModule, { preserveState: true })
    register(this.$store)
    this.listarTarefas()
