  ...
  <div class="row">
    ...
    <div class="col-8">
      ...
      <ul class="list-group list-group-flush"> -->
        ...
        /> -->
        <!-- A classe active é um recurso que já vem com o bootstrap -->
        <FilmesListaIten
          v-for="filme in filmes"
          :key="filme.id"
          :filme="filme"
...
// Obs: A relacao de um componente pai para o filho, vice-versa, é algo unidirecional
// No caso, se um componente pai tiver mais de um filho, devido a esse fluxo, o componente filho desse pai
// não poderá passar dados para outo componente filho, mesmo que ambos sejam originados do mesmo componente pai
// Basicamente o diagrama dessa relacão unidirecional,se estabeleceda seguinte forma:
// Pai para filho, via props. Filho para pai, via Events. Visando que a hierarquia entre pai e filho, está nas alteracoes que podem ser feita ou não.
// Em outras palavras, se ocorrer alguma alteracao de dados no filho que pode dar algum conflito com alguma alteracao similiar feita no pai, a alteracao será prioritário para o pai do que filho.
// Ou seja, o componente filho não consegue causar alguma mutacao no componente pai, mas o pai consegue para o filho. E foi visto que, se por acaso quisermos que o componente filho realize alguma mutacao no componente pai, serã necessário usar o event, como foi abordado nas aulas via o uso do this.$emit
// Logo, com isso, se quisermos passar algum dado de um componente filho para o outro filho, sendo ambos do mesmo componente pai, serã necessário fazer o fluxo
// pai para filho 1, via prop, filho1 para pai, via event sem causar alteraca da prop pai para filho inicial, e, depois, pai para filho2
// Vale ressaltar que existe uma outra forma de passar algum dado de componente filho para o pai, sem que seja via eventos (event). No caso, usa-se o call back, que é um método.
export default {
  components: {
    FilmesListaIten,
    FilmesListaItenInfo,
  ...
  methods: {
    aplicarClasseAtiva (filmeIterado) {
      // Essa é a forma como se chama uma classe de css para aplicacao de uso condicionais, mas a classe teria que estar combiando com o : (v-bind)
      // No caso, dentro da a instância class, combinado com o v-bind, será chamado uma funcao de dento dessa funcao será retornado uma as classes existentes (neste caso, o active) para combinar com as condicionais lógicos para o seu uso
      return {
        active: this.filmesSelecionado && this.filmesSelecionado.id === filmeIterado.id
      }
    },
    editarFilme (filme) {
      this.editar = true
    ...
    atualizarFilme (filmeAtualizado) {
      // o método findIndex serve para percorrer e para cada iteracao
      // e dara como parametro no call back que eu posso executar o filme
