...
<script>
// O curso de Vue Router, basicamente, todos os seus conteúdos que serão abordados estarão contidos dentro desse link https://router.vuejs.org/introduction.html
...
// Nessa trigésima e oitava aula, vamos aprender a usar os fragmentos Hashs que tem no vue router
// No caso, no arquivo ContatoDetalhes vamos aplicar isso na div criada com o style de height com 900 px e na tag p podemos definir um id com o nome parametros
// No caso, o id definido acima, na rota, ao colocarmos na forma contatos/1#parametros, o scroll irá se iniciar diretamente no parágrafo que está lá em baixo, onde mandamos pelo style com height de 900px
// De maneira análoga, podemos realizar a mesma coisa no arquivo ContatosListaItens no router-link onde tem o :to definido com a hash que nos evita em ter que colocar o #parametros manualmente com mais alguns recursos que será visto na próxima aula!
// Nessa trigésima e nona aula, vamos aprender a controlar o comportamento de scroll via o hash do vue router
// No caso, no arquivo index.js, na constante router teremos que definir um método chamado scrollBehavior
// Nessa quadragésima aula, vamos aprender a aplicar um recurso chamado lazy loading que nada mais é que um carregamento sob demanda
// No caso, no arquivo index.js vamos aplicar tal recurso usando os recursos import() aplicados junto com a const para cada component importado
// Nessa quadragésima e primeira aula, seguir o link sobre dicas para formas de buscar dados ao ativar determinada rota  https://router.vuejs.org/guide/advanced/data-fetching.html
// Nessa quadragésima e segunda aula, vamos aprender a centralizar dados e métodos no EventBus, mas como nas aulas anteriores, a implementação do EventBus não foi bem sucedido por conta da incompatibilidade da versão do vue, deixarei essa aula como um conteúdo em aberto para que vc aprenda particularmente ela, via outro projeto, com aplicação compatível com a versão do vue
// Nessa quadragésima e terceira aula, vamos aprender a exibir mais detalhes dos contatos. No caso, no component ContatoDetalhes vamos fazer uma limpeza nela
// Entretanto, essa aula é necessário que o event-bus esteja funcionando para que podemos executar a forma dos detalhes que queremos realizar. Como a incompatibilidade não nos permite isso, então vamos deixar isso para o estudo futuro
// Aula - 44. pegar o código fonte da seção!
</script>
