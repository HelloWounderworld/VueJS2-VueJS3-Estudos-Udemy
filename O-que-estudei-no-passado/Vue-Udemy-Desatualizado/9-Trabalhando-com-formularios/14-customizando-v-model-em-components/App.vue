          ...
          <form @submit.prevent="enviar" @reset="resetar">
            ...
            <div class="form-group">
              <rangeBtn
                ...
                max="15000"
                step="500"
                :inputClasses="[{'form-control-range': true}, 'minha-classes']" />
            </div>
...
// Vamos agora aprender a manipular a submissão do formulário, usando o botão Enviar já definido.
// Vamos criar um método chamado enviar para linkar com o evento no botão
// Uma outra forma de fazermos o mesmo evento é usando a opção submit dentro do type, do botão enviar, e linkar isso com @submit="enviar" colocado dentro da tag form
// Mas o evento que será captado dentro do método definido enviar, não será mais o evento de click
// Ao clicar no botão enviar, ele dará um refresh, isso está atrelado ao compartamento padrão nativa do html.
// Poderiamos, por exemplo, indicar pela propriedade action (<form @submit="enviar" action="">) e colocar uma url para manipular o server side. Mas como estamos uasndo o vue, queremos usar isso para o client side
// Para isso, podemos usar o modificador prevent, dentro da @submit, e isso fará com que funcione como o evento click definido para enviar
// Em algumas situações, precisará fornecer um botão para que o usuário precise resetar o formulário
// Para isso, precisamos acrescentar o type="reset" dentro do botão Resetar
// Obs: Quando um botão está dentro do fomulário submite sem nenhum type, automaticamente, será considerado como um botão de submit
// Porém, da forma como está o botão Resetar, ele limpará, literalmnte, tudo que está preenchido, ignorando os padrões que já estavam pré definidas
// Para que, ao clicar no botão Resetar, voltar ao tipo de padrão inicial definido vamos ter que definir mais algumas propriedades e métodos, valoresPadroes e resetar, junto com o lifecycle hook created
// Por fim, vc terá que criar no @reset linkando com o método resetar para conseguir fazer o que precisamos
// Vimos que a diretiva v-model, ela ligará os valores de forma bidirecional
export default {
  components: {
    rangeBtn
  },
